 ...
 * 获取style样式
 */
_utils.getStyle = (element, attr, NumberMode = 'int') => {
  let target
 ...
 * 页面到达底部，加载更多
 */
_utils.loadMore = (element, callback) => {
  ...
  element.addEventListener(
    ...
    () => {
      ...
      paddingBottom = _utils.getStyle(element, 'paddingBottom')
      marginBottom = _utils.getStyle(element, 'marginBottom')
    },
    { passive: true }
  )
...
_utils.animate = (
  ...
  const attrStyle = attr => {
    if (attr === 'opacity') {
      return Math.round(_utils.getStyle(element, attr, 'float') * 100)
    } else {
      return _utils.getStyle(element, attr)
    }
  }
